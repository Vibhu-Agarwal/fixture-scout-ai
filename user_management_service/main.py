...
DATABASE_NAME = os.getenv("FIRESTORE_DATABASE_NAME", "fixture-scout-ai-db")

# Initialize Firestore client
# If running locally and GOOGLE_APPLICATION_CREDENTIALS is set, it uses that.
# If on Cloud Run, it uses the runtime service account.
...
class UserPreferenceSubmitRequest(BaseModel):
    ...
    optimized_llm_prompt: str = Field(..., examples=["Remind me of all Real Madrid matches and important Champions League games involving top teams."])
...
async def get_user_preferences(
    ...
    if not preference_doc.exists:
        raise HTTPException(status_code=404, detail=f"Preferences for user ID {user_id} not found. Please submit preferences first.")
...
@app.get("/")
async def read_root():
    return {"message": "Welcome to the Fixture Scout AI - User Management & Preference Service"}
