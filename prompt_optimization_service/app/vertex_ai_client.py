    ...
    if _optimizer_gemini_model_instance is None:
        try:
            if not settings.GCP_PROJECT_ID:
                ...
                logger.warning(
                    "GCP_PROJECT_ID not explicitly set for Vertex AI client initialization."
                )
            ...
            if settings.GCP_REGION:
                ...
                logger.info(
                    f"Optimizer Vertex AI initialized for project '{settings.GCP_PROJECT_ID}' and location '{settings.GCP_REGION}'."
                )
            else:
                vertexai.init(project=settings.GCP_PROJECT_ID)
                logger.info(
                    f"Optimizer Vertex AI initialized for project '{settings.GCP_PROJECT_ID}' (no specific location)."
                )

            _optimizer_gemini_model_instance = GenerativeModel(
                settings.OPTIMIZER_GEMINI_MODEL_NAME
            )
            logger.info(
                f"Optimizer Vertex AI Gemini model '{settings.OPTIMIZER_GEMINI_MODEL_NAME}' loaded."
            )
