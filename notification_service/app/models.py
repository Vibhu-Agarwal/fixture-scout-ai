...
class NotificationLogDoc(BaseModel):
    notification_log_id: str = Field(default_factory=lambda: str(uuid.uuid4())) # Auto-generate
    original_reminder_id: str
    user_id: str
    reminder_mode: str
    ...
    last_attempt_utc: datetime.datetime = Field(default_factory=lambda: datetime.datetime.now(datetime.timezone.utc))
    created_at: datetime.datetime = Field(default_factory=lambda: datetime.datetime.now(datetime.timezone.utc))
    processed_at: Optional[datetime.datetime] = None
    final_status_published_at: Optional[datetime.datetime] = None # When status was sent to status topic
