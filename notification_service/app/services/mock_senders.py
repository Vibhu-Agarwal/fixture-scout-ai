...
class INotificationSender(Protocol):
    async def send(self, payload: PubSubMessageData) -> tuple[bool, str]: # (success, status_message)
        ...

class MockEmailSender:
    async def send(self, payload: PubSubMessageData) -> tuple[bool, str]:
        if not payload.contact_email:
            logger.error(f"MOCK EMAIL: No email address for user {payload.user_id}, reminder {payload.original_reminder_id}")
...
class MockPhoneCallSender:
    async def send(self, payload: PubSubMessageData) -> tuple[bool, str]:
        if not payload.contact_phone:
            logger.error(f"MOCK PHONE: No phone number for user {payload.user_id}, reminder {payload.original_reminder_id}")
