...
async def process_fixtures_for_user(user_id: str) -> Dict:
    ...
    recent_negative_feedback: List[ScoutUserFeedbackDoc] = []
    try:
        feedback_query = (
            db.collection(settings.USER_FEEDBACK_COLLECTION)
            .where("user_id", "==", user_id)
            # .where("is_interested", "==", False) # This field is in UserFeedbackDoc, not needed if filtering by collection of negative feedback
            .order_by("timestamp", direction=firestore.Query.DESCENDING)
            .limit(
                3
            )  # Fetch, e.g., the last 3-5 pieces of feedback. More can make prompt too long.
            .stream()
        )
        for fb_snap in feedback_query:
            try:
                # Ensure the data has 'fixture_details_snapshot' before parsing
                feedback_data = fb_snap.to_dict()
                if (
                    "fixture_details_snapshot" in feedback_data
                    and feedback_data["fixture_details_snapshot"] is not None
                ):
                    recent_negative_feedback.append(
                        ScoutUserFeedbackDoc(**feedback_data)
                    )
                else:
                    logger.warning(
                        f"Scout: Feedback doc {fb_snap.id} for user {user_id} missing 'fixture_details_snapshot'. Skipping this feedback item."
                    )
            except ValidationError as e:
                logger.warning(
                    f"Scout: Invalid feedback doc {fb_snap.id} for user {user_id}: {e}"
                )
    except Exception as e:
        logger.error(
            f"Scout: Error fetching user feedback for {user_id}: {e}", exc_info=True
        )
    ...
    full_llm_prompt = construct_gemini_scout_prompt(
        optimized_llm_prompt_text, upcoming_fixtures_for_llm, recent_negative_feedback
    )
    logger.debug(
...
def _store_new_reminders(
    ...
    optimized_llm_prompt_text: Optional[str] = None,
) -> int:
        ...
        for trigger in llm_match_info.reminder_triggers:
            ...
            try:
                reminder_data = ReminderDoc(
                    ...
                    optimized_llm_prompt_snapshot=optimized_llm_prompt_text,
